...
/**
 * Every derivative is its master, resized to one step of the ladder — and the files say so.
 *
 * The weight guard next door measures what these photographs cost; nothing measured what they
 * *are*. A derivative is generated by a tool that runs outside the build, from a master the build
 * never opens, so the two ways it goes wrong are invisible in review and in the page: a ladder
 * change with the files left behind (every candidate present, every candidate the old size), and a
 * master that is not the shape `src/data.js` declares for it — which reserves the wrong box on the
 * page and was a real bug here, one master spending its life claiming to be 640x960.
 *
 * So this reads the containers and compares them with the masters: each derivative's canvas must be
 * the ladder width at the master's own aspect ratio, with no alpha channel, because these are
 * opaque photographs, and one frame, because a page image is. The pixels are not decoded — they
 * are VP8, and a decoder is the kind of dependency this project does not take — which is why the
 * comparison is the shape of the file rather than its contents.
 *
 * `served` is `[{ path, width, master }]`, `masters` is the photographs themselves; both come from
 * src/photos.js in the build and can be handed in crafted, which is how it is tested.
 */
export function assertPhotographsMatchTheirMasters(served, masters) {
  const failures = [];
  const sizes = new Map(masters.map(photo => {
    const real = masterSize(photo.master);
    if (real.width !== photo.width || real.height !== photo.height) {
      failures.push(`${photo.master} is ${real.width}x${real.height} on disk and src/data.js declares ${photo.width}x${photo.height}, so the page reserves the wrong box for it`);
    }
    return [photo.master, real];
  }));

  for (const { path, width, master } of served) {
    const image = readWebp(path);
    const source = sizes.get(master) ?? masterSize(master);
    // One pixel of latitude: the ladder is rounded in Python and read back here in JavaScript, and
    // a half pixel of agreement between two languages is not a photograph that needs regenerating.
    const height = Math.round((source.height * width) / source.width);
    if (image.width !== width || Math.abs(image.height - height) > 1) {
      failures.push(`${path} is ${image.width}x${image.height} and ${master} at the ladder's ${width}px is ${width}x${height}${image.container === 'VP8X' ? '' : ` as a ${image.container}`}`);
    }
    if (image.alpha) {
      failures.push(`${path} carries an alpha channel, and these photographs are opaque — convert to RGB before resizing`);
    }
    if (image.animated) {
      failures.push(`${path} is an animated WebP (${image.chunks.join(', ')}), and a page image is one frame`);
    }
  }

  if (failures.length) {
    throw new Error(`The published photographs are not the ladder they are served as: ${failures.join('; ')} — regenerate with npm run photos, or correct the master's declared size`);
  }
